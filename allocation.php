...
while($row=mysql_fetch_array($result)){
	...
	$pdf->SetXY(35.0,210.5);
	$pdf->SetFont('edusongbig5', '', 12, '', true);
	$pdf->Write(5, '墓基明細', '');
	$pdf->SetXY(35.0,215.5);
	$countbase=count($baseview);
	for($views=0;$views<$countbase;$views++){
		$pdf->Cell(50, 4, $baseview[$views], 0, 0, 'L', 0);
		$w++;
		if($w==4){$pdf->SetXY(35.0,215.5+$lineout);$w=0;$lineout=$lineout+5;}
	}
	...
	$pdf->SetXY(83.0,265.0);$pdf->Write(3,'8 1 9', '');
	$y=strlen($price);
	$v=182.5;
	for($c=1;$c<=$y;$c++){
