...
error_reporting(0);
$resetsql="delete from merage_roll";
...
while ($row=mysql_fetch_array($result)) {
	...
	for($s=0;$s<=count($a);$s++){
		if($row['roll_id']==$a[$s]){
			...
			$insertsql="INSERT into merage_roll(roll_id,base_id,area,rightuser,username,phone,address) values ($roll_id,'$base_id',$area,'$rightuser','$username','$phone','$address')";
	...
	if($c==0){
		...
		$roll_id=$row['roll_id'];$base_id=$row['base_id'];$area=$row['area'];$rightuser=$row['rightuser'];$username=$row['username'];$phone=$row['phone'];$address=$row['address'];
		$insertsql="INSERT into merage_roll(roll_id,base_id,area,rightuser,username,phone,address,price) values ($roll_id,'$base_id',$area,'$rightuser','$username','$phone','$address',$price)";
		$insertresult=mysql_query($insertsql)or die('Invalid query: ' . mysql_error());
		$i++;
		//首筆相同資訊加入並加總收費
	}
